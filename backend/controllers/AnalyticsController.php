...
class AnalyticsController extends AbstractController
{
     ...
     * @return string
     */
    public function actionGameStatistic()
    {
        $game = Game::find()
            ->joinWith(['schedule'])
            ->select([
                'penalty' => new Expression('AVG(`game_home_penalty`+`game_guest_penalty`)'),
                'score' => new Expression('AVG(`game_home_score`+`game_guest_score`)'),
                'shot' => new Expression('AVG(`game_home_shot`+`game_guest_shot`)'),
            ])
            ->where(['!=', 'game_played', 0])
            ->andWhere('`schedule_date`>UNIX_TIMESTAMP()-604800')
            ->one();
    ...
    public function actionSnapshot($id = 1)
    {
