...
function f_igosja_generator_president_fire()
{
    ...
    foreach ($country_array as $item)
    {
        ...
        if ($rating_negative > 25)
        {
            ...
            $news_text = 'Действующий президент федерации отправлен в отставку по причине высокого уровня недоверия менеджеров федерации.';

            if ($item['country_vice_id'])
            {
                $news_text = $news_text . ' Заместитель президента занял вакантную должность.';
            }
            else
            {
                $news_text = $news_text . ' Поскольку заместителя у президента не было, должность стала вакантной.';
            }
