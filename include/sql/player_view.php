...
$sql = "SELECT `ratingchangeplayer_power`,
               `ratingchangeplayer_schedule_id`
        FROM `ratingchangeplayer`
        WHERE `ratingchangeplayer_player_id`=$num_get
        ORDER BY `ratingchangeplayer_schedule_id` ASC";
$player_rating_sql = f_igosja_mysqli_query($sql);

$player_rating_array = $player_rating_sql->fetch_all(MYSQLI_ASSOC);

$player_start_schedule_id = $player_rating_array[0]['ratingchangeplayer_schedule_id'];
...
$player_schedule_sql = f_igosja_mysqli_query($sql);

$player_schedule_array = $player_schedule_sql->fetch_all(MYSQLI_ASSOC);
...
{
    foreach ($player_rating_array as $rating)
    {
        if ($item['schedule_id'] == $rating['ratingchangeplayer_schedule_id'])
        {
            $player_power = $rating['ratingchangeplayer_power'];
        }
    }
