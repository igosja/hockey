...
class GameSend extends Model
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $lineupArray = Lineup::find()
            ->where(['lineup_game_id' => $this->game->game_id, 'lineup_team_id' => $this->team->team_id])
            ->all();
    ...
    public function saveLineup()
    {
        ...
        for ($i = 0; $i < 22; $i++) {
            ...
            $player = Player::find()
                ...
                ->andWhere([
                    'or',
                    ['player_team_id' => $this->team->team_id, 'player_loan_team_id' => 0],
                    ['player_loan_team_id' => $this->team->team_id]
                ])
                ->limit(1)
                ->one();
            if (!$player) {
            ...
            $lineup = Lineup::find()
                ->where([
                    ...
                    'lineup_team_id' => $this->team->team_id,
                ])
                ->limit(1)
                ->one();
            if (!$lineup) {
