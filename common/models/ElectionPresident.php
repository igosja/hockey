...
class ElectionPresident extends AbstractActiveRecord
{
    ...
    public function applications(): array
    {
        ...
        foreach ($this->application as $application) {
            ...
            $result[] = [
                'count' => $count,
                'user' => $application->election_president_application_user_id ? $application->user->userLink() : 'Против всех',
                'logo' => $application->election_president_application_user_id ? $application->user->smallLogo() : '',
            ];
            $total = $total + $count;
        }
    ...
    {
        return $this->hasOne(Country::class, ['country_id' => 'election_president_country_id'])->cache();
    }
